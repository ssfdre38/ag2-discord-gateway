 ...
 * TypeScript Port of haven-cpp / haven_memory.h & haven_memory.cpp
 * ══════════════════════════════════════════════════════════════════════
 * Provides:
...
export const HMB_MAGIC = "HAVENMEM"; // 8 bytes (0x484156454E4D454D)
export const HMB_VERSION = 0x00020000; // 64-bit v2.0
export const HEADER_SIZE = 136; // 136 bytes
export const RECORD_SIZE = 84; // 84 bytes
...
export interface MemoryAnchor {
  ...
  concept_name: string;
  text_content: string;
  category: "CORE_IDENTITY" | "EPISODIC" | "SEMANTIC" | "EMOTIONAL" | string;
  weight: number; // Salience / Importance (0.0 to 1.0)
  emotional_salience: number; // Emotional resonance (0.0 to 1.0)
  timestamp: number | bigint; // Unix Epoch microseconds
  access_count: number | bigint; // Access frequency counter
  embedding: Float32Array; // Dense semantic vector
}
...
export class HmbMemoryEngine {
  private memories: MemoryAnchor[] = [];
  private workingMemoryBuffer: WorkingTurn[] = [];
  private workingBufferSize: number = 8;
  private embeddingDim: number = DEFAULT_EMBEDDING_DIM;
  private nextId: bigint = 1001n;
  private vaultPath: string = "";

  constructor(options?: {
    embeddingDim?: number;
    workingBufferSize?: number;
    vaultPath?: string;
  }) {
    if (options?.embeddingDim) this.embeddingDim = options.embeddingDim;
    if (options?.workingBufferSize) this.workingBufferSize = options.workingBufferSize;
    if (options?.vaultPath) this.vaultPath = options.vaultPath;
  }

  // ─── L0 Working Memory Ring Buffer ───────────────────────────────────────
  public pushTurn(role: "user" | "assistant" | "system", author: string, content: string) {
  ...
  public getWorkingMemory(): WorkingTurn[] {
    return [...this.workingMemoryBuffer];
  }

  // ─── L1 / L2 Anchors Management ──────────────────────────────────────────
  public addMemory(anchor: Partial<MemoryAnchor> & { concept_name: string; text_content: string }): MemoryAnchor {
    const id = anchor.id !== undefined ? BigInt(anchor.id) : this.nextId++;
    if (id >= this.nextId) this.nextId = id + 1n;

    const fullAnchor: MemoryAnchor = {
      id,
  ...
  public getAllMemories(): MemoryAnchor[] {
  ...
  public buildContextInjection(userPrompt: string, topK: number = 3): string {
    const recalled = this.searchTopK(userPrompt, topK);
    if (recalled.length === 0) return "";

    let text = "\n[Hierarchical Memory Buffer (HMB) Context]:\n";
    for (const item of recalled) {
      const m = item.anchor;
      text += `• [${m.category}] ${m.concept_name}: ${m.text_content} (Salience: ${m.weight.toFixed(2)}, Score: ${item.score.toFixed(2)})\n`;
    }
  ...
  public initializeCoreIdentity(botName: string = "Ash"): void {
    if (this.memories.length > 0) return;
  ...
  public saveToHmb(filepath: string): boolean {
    try {
      ...
      const vectorTableSize = total * dim * 4; // 4 bytes per float
      ...
      // reserved bytes [72..135] remain 0
  ...
  public loadFromHmb(filepath: string): boolean {
    try {
      ...
      const newMemories: MemoryAnchor[] = [];
  ...
  // ─── JSON Export / Import ────────────────────────────────────────────────
  public saveToJson(filepath: string): boolean {
    try {
      const data = {
        version: 2,
        engine: "hmb-typescript-64bit",
  ...
  public loadFromJson(filepath: string): boolean {
    try {
      if (!fs.existsSync(filepath)) return false;
